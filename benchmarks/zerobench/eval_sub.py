# -*- coding: utf-8 -*-
"""子问题集评测：MiMo 直答 pass@5 + 断点续跑
用法: python zb_sub.py <start> <end> [--samples N]
"""
...
OUTDIR = r'C:\Users\Adfhj\Desktop\OH-WorkSpace\zerobench\imgs_sub'
RESULT = r'C:\Users\Adfhj\Desktop\OH-WorkSpace\zerobench\results_sub.json'
MAX_SIDE = 1280


def load_data():
    import pyarrow.parquet as pq
    t = pq.read_table(r'C:\Users\Adfhj\Desktop\OH-WorkSpace\zerobench\zerobench_sub.parquet')
    print('子问题总数:', t.num_rows)
    print('列:', t.column_names)
    rows = []
...
def judge(pred, answer, qtype):
    """按类型判分：numeric 数字匹配；mcq/yes_no/open 在 FINAL ANSWER 后找答案字符串。"""
    if qtype == 'numeric':
        return judge_numeric(pred, answer)
    # mcq / yes_no / open：FINAL ANSWER 之后的内容包含答案（大小写不敏感，忽略标点）
    tail = pred.split('FINAL ANSWER', 1)[-1] if 'FINAL ANSWER' in pred else pred
    norm = lambda s: re.sub(r'[^a-z0-9\u4e00-\u9fff]', '', str(s).lower())
    a = norm(answer)
    if not a:
        return False
    return a in norm(tail)
    ...
    for i in range(args.start - 1, min(args.end, len(rows))):
        ...
        for s in range(args.samples):
            ...
            try:
                ...
                preds.append(p)
                print('  [%d] %.1fs ...%s' % (s + 1, time.time() - t0, p[-50:].replace(chr(10), ' ')), flush=True)
                if judge(p, row['answer'], row['type']):
                    correct = True
                    print('  >>> 命中!', flush=True)
                    break
        ...
        if correct is None and row['type'] == 'numeric':
            correct = False
        new_results.append({'id': row['id'], 'type': row['type'], 'answer': row['answer'],
        ...
        if os.path.exists(RESULT):
            try:
                ...
                pass
        old = [r for r in old if str(r['id']) != str(row['id'])] + new_results
        with open(RESULT, 'w', encoding='utf-8') as f:
            json.dump(old, f, ensure_ascii=False, indent=1)

    allr = json.load(open(RESULT, encoding='utf-8')) if os.path.exists(RESULT) else []
    ok = sum(1 for r in allr if r['correct'])
    total = sum(1 for r in allr if r['correct'] is not None)
    print()
    print('=== 子问题累计: %d/%d ===' % (ok, total))
